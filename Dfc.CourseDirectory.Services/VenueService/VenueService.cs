...
namespace Dfc.CourseDirectory.Services.VenueService
{
    public class VenueService : IVenueService
    {
        ...
        private readonly Uri _getAllVenues;
            ...
            IOptions<VenueServiceSettings> settings)
        {
            ...
            _getAllVenues = settings.Value.ToGetAllVenues();
        }
        ...
        public async Task<Result<List<Venue>>> GetAllVenues()
        {
            try
            {
                var response = await _httpClient.GetAsync(new Uri(_getAllVenues.AbsoluteUri));
                ...
                {
                    return Result.Fail<List<Venue>>("Unsuccessful http response");
                }
            }
            catch (HttpRequestException hre)
            {
                _logger.LogException("Get all venues service http request error", hre);
                return Result.Fail<List<Venue>>("Get all venues service http request error.");

            }
            catch (Exception e)
            {
                _logger.LogException("Get all venues service unknown error.", e);
                return Result.Fail<List<Venue>>("Get all venues service unknown error.");
